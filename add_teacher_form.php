    ...
    <?php
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($res)
            {
                ...
                {
                    $sql1="INSERT INTO `user` (`id`, `username`, `name`, `phone`, `email`, `password`, `usertype`) VALUES (NULL, '$username', '$name', '$phone', '$email', '$password', 'teacher') ";
                    $result1=mysqli_query($conn,$sql1);
        
                    if($result1)
                    {
                        echo "<p style='color: green'> Teacher added to database successfully ! </p>";
                    }
                    else
                    {
